...
const CONFIG = {
  ...
  UPDATES_CHANNEL_ID: process.env.UPDATES_CHANNEL_ID, // Channel for website post announcements
...
async function createPostFromDiscord(message, belongingUser, discordUser) {
  try {
    ...
    if (belongingUser) {
      ...
      const [postResult] = await pool.execute(
        'INSERT INTO posts (user_id, tagline, content) VALUES (?, ?, ?)',
        [1, tagline, content] // Use admin user as placeholder
      );
...
async function createCommentFromDiscord(message, parentPostId, belongingUser, discordUser) {
  try {
    ...
    if (belongingUser) {
      ...
      commentUserId = 1; // Admin placeholder
    }
...
async function postToDiscord(post, user) {
  try {
    ...
    // Also post to updates channel if configured
    if (CONFIG.UPDATES_CHANNEL_ID && CONFIG.UPDATES_CHANNEL_ID !== CONFIG.WEBSITE_CHANNEL_ID) {
      try {
        const updatesChannel = await client.channels.fetch(CONFIG.UPDATES_CHANNEL_ID);
        if (updatesChannel) {
          await rateLimitQueue.add(async () => {
            return await updatesChannel.send({ embeds: [embed], components: [row] });
          });
          console.log(`Also posted to updates channel for post ${post.id}`);
        }
      } catch (err) {
        console.error(`Failed to post to updates channel:`, err);
      }
    }
