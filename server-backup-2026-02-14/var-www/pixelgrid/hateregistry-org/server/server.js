...
// Get all people (admin - includes all statuses)
app.get('/api/admin/people', requireAdmin, async (req, res) => {
  try {
    const [people] = await pool.query(
      `SELECT p.*, u.username as submitted_by_username,
       (SELECT COUNT(*) FROM comments WHERE person_id = p.id) as comment_count,
       (SELECT COUNT(*) FROM hate_records WHERE person_id = p.id) as record_count
       FROM people p 
...
// Get all comments (admin - includes all statuses)
...
app.delete('/api/admin/people/:id', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    await connection.execute('DELETE FROM hate_records WHERE person_id = ?', [id]);
    await connection.execute('DELETE FROM social_profiles WHERE person_id = ?', [id]);
    await connection.execute('DELETE FROM comments WHERE person_id = ?', [id]);
    await connection.execute('DELETE FROM photos WHERE person_id = ?', [id]);
...
app.post('/api/people', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO people (first_name, middle_name, last_name, slug, job, phone_number, location, family_members, description, profile_photo_url, social_link, markup_content, status, submitted_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [first_name || '', middle_name || null, last_name || '', slug, job || null, phone_number || null, location || null, family_members || null, description || null, profile_photo_url || null, social_link, markup_content || null, status, submittedBy]
    );
...
app.get('/sitemap.xml', async (req, res) => {
  try {
    ...
    const links = [
      { url: '/hate-registry/', changefreq: 'daily', priority: 1.0 }
    ];
    ...
    const stream = new SitemapStream({ hostname: 'https://hateregistry.org' });
...
// Start server
app.listen(PORT, () => {
  console.log(`Hate Crime Registry API server running on port ${PORT}`);
});
