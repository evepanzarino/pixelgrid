...
app.get('/api/auth/me', requireAuth, async (req, res) => {
  try {
    const [users] = await pool.execute(
      'SELECT id, username, email, first_name, last_name, role, profile_picture, discord_id, discord_username, discord_avatar, created_at FROM users WHERE id = ?',
      [req.user.id]
    );
...
app.get('/api/posts', authenticateToken, async (req, res) => {
  try {
    ...
    if (tab === 'personal' && userId) {
      ...
      // Updates feed: posts from 'belonging' user (ID 2)
      query = `
        SELECT p.*, u.username, u.first_name, u.last_name, u.profile_picture,
          dps.discord_thread_id,
          (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comment_count,
...
app.post('/api/posts/:id/like', requireAuth, async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      ...
      // Award XP to author if not self-like
      ...
      const [count] = await pool.execute('SELECT COUNT(*) as count FROM post_likes WHERE post_id = ?', [postId]);
      return res.json({ liked: true, like_count: count[0].count });
    }
  } catch (error) {
...
const SKILLS = {
  posting: { name: 'Posting', xpPerAction: 50 },
  messaging: { name: 'Messaging', xpPerAction: 25 },
  commenting: { name: 'Commenting', xpPerAction: 35 },
  being_liked: { name: 'Being Liked', xpPerAction: 10 },
  being_favorited: { name: 'Being Favorited', xpPerAction: 15 },
  being_reposted: { name: 'Being Reposted', xpPerAction: 25 }
};
