...
import { getPerson, createComment } from '../api';
...
function PersonPage() {
  ...
  if (error) {
    return (
      <>
        <SEOHead title="Not Found - Bigot Registry" />
  ...
  return (
    <>
      <SEOHead
        title={`${fullName} - Bigot Registry`}
        description={person.description || `Profile for ${fullName} in the Bigot Registry`}
        canonical={`https://evepanzarino.com/bigot-registry/person/${person.slug}`}
        type="profile"
      />
