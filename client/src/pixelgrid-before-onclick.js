...
export default function PixelGrid() {
  ...
  function extractLayerToSelected(layerName) {
    ...
    if (!layer || !layer.pixels) return null;
    ...
    // Update groups: clear original layer and add __selected__
    setGroups(prevGroups => {
      return prevGroups
        .filter(g => g.name !== "__selected__")
        .map(g => g.name === layerName ? { ...g, pixels: {} } : g)
        .concat([selectedLayer]);
    });

    // Update pixelGroups to point to __selected__
    const updatedPixelGroups = { ...pixelGroups };
    Object.keys(layer.pixels).forEach(pixelIndex => {
      updatedPixelGroups[pixelIndex] = { group: "__selected__", zIndex: 9999 };
    });
    setPixelGroups(updatedPixelGroups);

    return {
      layerName,
      pixelIndices: Object.keys(layer.pixels).map(idx => parseInt(idx)),
      zIndex: layer.zIndex
    };
  }
  ...
  function restoreSelectedToLayer() {
    ...
    // Update groups: restore data to original layer and remove __selected__
    setGroups(prevGroups => {
      return prevGroups
        .filter(g => g.name !== "__selected__")
        .map(g => {
          if (g.name === originalLayerName) {
            return { ...g, pixels: selectedLayer.pixels, zIndex: originalZIndex };
          }
          return g;
        });
    });

    // Update pixelGroups back to original layer
    const updatedPixelGroups = { ...pixelGroups };
    Object.keys(selectedLayer.pixels).forEach(pixelIndex => {
      updatedPixelGroups[pixelIndex] = { group: originalLayerName, zIndex: originalZIndex };
    });
    setPixelGroups(updatedPixelGroups);

    // Keep layer active
    setActiveGroup(originalLayerName);
  }
