...
export default function PixelGrid() {
  ...
  const getZoomFactor = () => {
    if (size.w <= 768) return 3; // Mobile
    if (size.w <= 1024) return 1.75; // Tablet
    if (size.w <= 1750) return 1.35;
    return 1.25; // Desktop
  };
        ...
        <div style={{
          ...
          >
            ...
            >
              ...
              <div style={{
                position: "absolute",
                left: `${gridRef.current ? Math.min(100 - (8 / (parseFloat(getComputedStyle(document.documentElement).width) * 0.01 * (100 - 4))) * 100, Math.max(0, (scrollPosition / (gridRef.current.scrollWidth - gridRef.current.clientWidth)) * 100)) : 0}%`,
                top: "0",
                width: "8vw",
                height: "11vw",
                background: "#000000",
